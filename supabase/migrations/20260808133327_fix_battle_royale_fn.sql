-- HistoryGuessr · OPRAVA · chybějící funkce advance_battle_royale
--
-- Zjištěno při review multiplayeru: sloupce multiplayer_players.eliminated
-- na produkci JSOU, ale funkce advance_battle_royale CHYBÍ (nedostala se do
-- baseline snímku produkce). Klient ji pro Battle Royale volá → RPC selže →
-- kolo se po prvním kole NEPOSUNE a hra se zasekne (pro BR není fallback).
--
-- Tato migrace ji (znovu) vytvoří. Idempotentní (create or replace + IF NOT
-- EXISTS). Bezpečné pustit i tam, kde už by existovala.
