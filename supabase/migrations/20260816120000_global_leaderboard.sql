-- Globální žebříček hráčů (Fáze A). profiles má RLS „jen vlastní řádek",
-- proto SECURITY DEFINER — vystavíme jen bezpečnou veřejnou podmnožinu
-- (username, level/xp, skóre, počet her). Nikdy e-mail.

-- Top N hráčů podle celkového skóre.
create or replace function public.global_leaderboard(p_limit int default 50)
returns table(rank int, user_id uuid, username text, xp bigint, total_score bigint, games_played int)
language sql
security definer
stable
...
as $$
  select (row_number() over (order by pr.total_score desc, pr.xp desc))::int as rank,
         pr.id, pr.username, pr.xp, pr.total_score, pr.games_played
...
-- „Moje okolí" — já ± radius řádků, i když jsem mimo top N.
...
as $$
  ...
  select r.rank, r.id, r.username, r.xp, r.total_score, r.games_played
    from ranked r, me
