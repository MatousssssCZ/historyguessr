...
export default function EditorPage() {
  ...
  const load = useCallback(async () => {
    await releaseStaleTasks()   // vrať opuštěná zadání do zásobníku
  ...
  async function closeForm() {
    // Uvolní zadání zpět do zásobníku, pokud se nic neuložilo (no-op jinak).
    if (active) await releaseTask(active.task.id)
    setActive(null); load()
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--paper-200)' }}>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '28px 24px 48px' }}>
        ...
        <EditorLeaderboard meId={user?.id}/>
