...
Návrh je organizovaný do „turnů" (sekce `#t1`…`#t10`), každý turn = jedno kolo iterace s uživatelem. Nejnovější/platné jsou turny s vyšším číslem. Uvnitř jsou varianty s ID jako `4a`, `4c`, `6a`, `6b`, `8a`, `9b`, `10a` (písmeno `a`/`b`/`c`/`d` = mobil/desktop nebo krok).
...
### 10) Domů — doplněk: vstup do nápovědy
Na obrazovce Domů (`#4a`) je vedle avataru v headeru malá kulatá ikona `?` (`FBF7F0` pozadí, `muted` ikona) vedoucí na krok 1 onboardingu (`#10a`). Slouží novým hráčům, kteří ještě nevědí, že nápověda existuje, aniž by zabírala místo v hlavní navigaci nebo Play launcheru.

### 11) Potvrzovací e-mail po registraci — `#7a` (e-mail, 600px) + `#7b` (návazná obrazovka)
...
- **Disclaimer (`#7b`, pod formulářem):** tichý info box (ikona `info`, neutrální pozadí `#EFE7DA`, drobný text `10px`) s textem *„HistoryGuessr je fanouškovský projekt. Historické události mohou obsahovat nepřesnosti — berte je s rezervou."* Umístěn pod tlačítkem „Přihlásit se" a odkazem na registraci — nekonkuruje hlavnímu CTA, ale je vždy viditelný bez scrollu na prvním přihlášení po registraci.

### 12) Přihlášení / Registrace (`/login`, `/register`) — desktop split `#8a`/`#8b`, mobil `#8c`
...
### 13) Zkušební kolo bez registrace (`/try`) — mobil `#9a` (intro) + `#9b` (výsledek)
...
- **9b Výsledek zkušebního kola:** stejná struktura jako produkční Výsledek (`#6b`) — ikonka domů (`house`) vpravo nahoře vedle eyebrow labelu „ZKUŠEBNÍ KOLO" (vede zpět na Domů `#4a`), název události + skóre, tabs Skóre/O události, mapa s piny + spojnicí a popisky, karty POLOHA/ROK s progress bary, řádek SPRÁVNÝ ROK/TVŮJ TIP.
  ...
  - **Ikona domů** (`house`, kulatá) vpravo nahoře vedle eyebrow labelu „ZKUŠEBNÍ KOLO" — tichý únikový bod zpět na Domů (`#4a`) pro nepřihlášeného návštěvníka, který si to chce jen rozmyslet.

### 14) Onboarding „Jak se to hraje?" (`/how-to-play`) — mobil `#10a`–`#10d`, 4 kroky
**Účel:** vysvětlit jádrovou herní smyčku novým hráčům za méně než 30 sekund, bez nutnosti číst manuál. Zobrazuje se automaticky při prvním spuštění appky a je kdykoliv znovu dostupný (viz vstupní body níže).
- **Struktura kroku:** header s odkazem „Přeskočit" (kromě posledního kroku) vpravo nahoře → vizuální ukázka (panorama / mapa / rok-slider / skóre karta) → accent ikona-dlaždice 52px → Newsreader nadpis (2 řádky) → krátký popisný text (`Hanken Grotesk`, `muted`, max ~230px šířky, na střed) → tečková navigace (aktivní tečka širší, accent) → CTA tlačítko (accent, plná šířka).
  1. **`#10a` Rozhlédni se** — panorama placeholder se šipkami rotace vlevo/vpravo, hint „Táhni prstem po obraze". Text: „Ocitneš se přímo na místě události" / vysvětlení hledání vizuálních stop.
  2. **`#10b` Umísti tip na mapě** — mapová placeholder s pinem a zoom ovladači. Text: „Klepnutím na mapu umísti svůj tip" / čím blíž, tím víc bodů.
  3. **`#10c` Odhadni rok** — karta s velkým číslem roku (Newsreader, accent) + posuvník −3000…2025. Text: „Posuvníkem nastav rok, kdy se to stalo".
  4. **`#10d` Sbírej body a postupuj** — ukázková skóre karta (873/1000, Poloha/Rok dílčí hodnoty, +95 XP pill). Text: „Sbírej body, level a odznaky". Poslední CTA = **„Začít hrát"** (místo „Další") → vede na Domů (`#4a`), místo tečkové navigace na další krok.
- **Vstupní body do onboardingu:**
  - Automaticky po prvním úspěšném přihlášení/registraci (než uvidí Domů poprvé).
  - Kdykoliv znovu: ikona `?` na Domů (`#4a`, vedle avataru) a karta **„Jak hrát?"** na obrazovce Můj účet (`#5c`, úplně nahoře nad PROFIL — ikona, titulek, popisek „Krátké intro do pravidel hry", šipka).
  - Onboarding **není** v hlavní navigaci ani v Play launcheru — je to jednorázová edukace, ne herní mód.
...
## Files
- `HistoryGuessr.dc.html` — kompletní hi-fi prototyp všech obrazovek (turny #t1–#t10). Otevřete v prohlížeči; relevantní varianty: Domů `#4a`/`#4c`, Play launcher `#4a`, Nastav si hru `#2a`/`#2b`, Hraní `#1b`, Výsledek `#6a`/`#6b`, Odznaky+Statistiky `#3a`/`#3b`, Přátelé `#5a`, Multiplayer `#5b`, Můj účet `#5c`, Potvrzovací e-mail `#7a`/`#7b`, Přihlášení/Registrace `#8a`/`#8b`/`#8c`, Zkušební kolo `#9a`/`#9b`, Onboarding „Jak hrát?" `#10a`–`#10d`. (Skryté varianty `#1a` KINO a `#1c` ATLAS jsou zavržené alternativní palety — ignorujte.)
